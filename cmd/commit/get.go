...
var getCmd = &cobra.Command{
	...
	ValidArgsFunction: getValidAdrgs,
	RunE:              getProcess,
}
...
func getValidAdrgs(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	if len(args) != 0 {
...
func getProcess(cmd *cobra.Command, args []string) (err error) {
	...
	log.Infof("Displaying commit %s", commit)
	if !common.WhatIf(log.ToContext(cmd.Context()), cmd, fmt.Sprintf("Showing commit %s", commit)) {
		return nil
	}
	...
	err = profile.Get(log.ToContext(cmd.Context()), cmd, repository.GetPath("commits", commit), &commits)
	if err != nil {
		return err
	}
	if len(commits.Values) == 0 {
		log.Infof("No commit found with hash %s", commit)
		return
	}
	log.Record("commit", commits.Values[0]).Debugf("Commit %s retrieved successfully", commit)
